...
function init() {
  ...
  camera = new THREE.PerspectiveCamera(50, 0.5 * aspect, 1, 100000);
  ...
  const rails = new THREE.Mesh(
    new THREE.BoxGeometry(10, 5, 140),
  ...
  carriage = new THREE.Mesh(
    ...
    new THREE.MeshBasicMaterial({ color: 0xff0015 }),
  );
  cameraPerspective.add(carriage);
  // mesh = new THREE.Mesh(
...
function render() {
  ...
  cameraPerspective.position.x = 68 * Math.cos(r);
